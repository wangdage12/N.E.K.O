...
return-ball 内部会附带一个 `neko-idle-thought-bubble` 容器。容器里分两层：`thought-items/cloud-thought-bubble.gif` 作为气泡背景 GIF，独立内容图作为 `neko-idle-thought-bubble-item` 叠在大气泡中心。气泡只作为视觉陪伴层，不参与点击、hover 或拖拽命中；实现上保持 `pointer-events: none`，避免遮挡猫 GIF 的正常交互。
...
4. `CAT1` 走路、伸懒腰和走路/伸懒腰 hover 暂停期间不显示。
...
8. 思考气泡在长按 pending / 拖拽 / CAT1 子动作期间隐藏，并且只由随机声音播放成功触发渐显渐隐；普通气泡显示 5s，ZZZ 气泡跟随本次睡眠音频播放时长。
